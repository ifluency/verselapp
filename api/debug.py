...
def dump_first_pages(pdf_bytes: bytes, pages=2, max_lines=220) -> str:
    out = []
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        if "multipart/form-data" not in content_type:
            self._send(400, "Envie multipart/form-data com campo 'file'.")
            return
        ...
        if "file" not in form:
            self._send(400, "Campo 'file' ausente.")
            return

        pdf_bytes = form["file"].file.read()
        txt = dump_first_pages(pdf_bytes, pages=3, max_lines=260)
