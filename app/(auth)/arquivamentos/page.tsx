...
export default function ArquivamentosPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!data) {
        setStatus("Endpoint /api/archive não retornou JSON. Verifique se api/archive.py existe no deploy e se você está acessando pelo domínio da Vercel (não localhost)." + (parsed.rawText ? `\n\nResposta: ${String(parsed.rawText).slice(0,120)}...` : ""));
        return;
      }
  ...
  async function presignAndDownload(runId: string) {
    ...
    try {
      ...
      if (!data) {
        setStatus("Endpoint /api/archive não retornou JSON." + (parsed.rawText ? ` Resposta: ${String(parsed.rawText).slice(0,120)}...` : ""));
        return;
      }
